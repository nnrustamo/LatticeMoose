...
void LatticeBoltzmannCore::generateMesh()
{
    /** 
...
void LatticeBoltzmannCore::stream()
{   
    /**
    ...
    torch::Tensor f_copy = _lattice._f.clone();
    for (/* do not use unsigned int */ int i = 1; i < _stencil._q; i++)
    {   
        torch::Tensor rolled_tensor = torch::roll(f_copy.index({torch::indexing::Slice(), 
        ...
        _lattice._f.index({torch::indexing::Slice(), torch::indexing::Slice(), torch::indexing::Slice(), i}) = rolled_tensor;
    }
...
void LatticeBoltzmannCore::BGKCollision()
{   
    /**
...
void LatticeBoltzmannCore::MRTCollision()
{   
    /**
...
void LatticeBoltzmannCore::equilibrium()
{   
    /**
    ...
    torch::Tensor comp2 =  0.5 * ((ex * ux + ey * uy + ez * uz) * (ex * ux + ey * uy + ez * uz)) / _lattice._c_s_4;
...
void LatticeBoltzmannCore::observables()
{
    /**
...
void LatticeBoltzmannCore::residual()
{   
    /**
     * Compute residual 
     */

    _lattice._u = torch::sqrt(_lattice._ux * _lattice._ux + \
                                 _lattice._uy * _lattice._uy + \
                                 _lattice._uz * _lattice._uz);
    
    double sumUsqareMinusUsqareOld = torch::sum(torch::abs(_lattice._u - _lattice._u_old)).item<double>();
...
void LatticeBoltzmannCore::wallBoundary()
{
    /**
     * Halfway bounce-back
     */
    ...
    {   
        ...
        auto lattice_slice = _lattice._f.index({torch::indexing::Slice(), torch::indexing::Slice(), torch::indexing::Slice(), index});
        auto bounce_back_slice = f_bounce_back.index({torch::indexing::Slice(), torch::indexing::Slice(), torch::indexing::Slice(), ic});

        //f_bounce_back.index({torch::indexing::Slice(), torch::indexing::Slice(), torch::indexing::Slice(), ic}) =
        //    _lattice._f.index({torch::indexing::Slice(), torch::indexing::Slice(), torch::indexing::Slice(), _stencil._op[ic].item<int64_t>()});
...
void LatticeBoltzmannCore::openBoundary()
{
    /**
...
void LatticeBoltzmannCore::setfSolidtoZero()
{
    /**
     * Set distribution functions to zero @ solid nodes
     */
    auto solid_mask = (_lattice._mesh == 0).unsqueeze(-1).expand({_lattice._nz, 
                        _lattice._ny, _lattice._nx, _lattice._q});;
    
    _lattice._f.masked_fill_(solid_mask, 0);
}


void LatticeBoltzmannCore::setfeqSolidtoZero()
{
    /**
...
void LatticeBoltzmannCore::setObservablestoZero()
{
    /**
...
void LatticeBoltzmannCore::setStencil(const StencilBase &stencil_new)
{   
    _stencil = stencil_new;
}

void LatticeBoltzmannCore::setLatticeBase(const LatticeBase &lattice_new)
{
    _lattice = lattice_new;
}

void LatticeBoltzmannCore::regularize()
{
    /**
